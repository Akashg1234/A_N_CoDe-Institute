...
<body>
   <div class="" style="float: right; margin-top: -150px;margin-right: 50px; ">
      ...
      <a href="new-adminpasswordupdate.php" id="fgtpass" >Forgot password</a>
   </div>
...
if(isset($_POST['loginbtn']))
{
   $id=$_POST["logid"];
   $pass=$_POST["pswd"];
   $q=" SELECT adminid,passw FROM adminreg WHERE adminid='$id'  ";
   ...
   if($res=mysqli_query($connect,$q))
   {
      ...
      if(is_array($val))
      {
         if(password_verify($pass, $val[1]))
         {

            $_SESSION['admin_id']=$val[0];
            $_SESSION['pass']=$val[1];
            echo "<script>window.location.href='admin-view.php';</script>";
