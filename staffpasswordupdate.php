...
<body>
   ...
   ?>
      <div style=" margin-top: 30px;">
         <a href="instructor-view.php" class="p-2" 
         style="
         margin-left: 575px;
         font-size:30px; 
         color: grey; 
   ...
   ?>
      <div style=" margin-top: 30px;">
         <a href="staff-view.php" class="p-2" 
         style="
         margin-left: 575px;
         font-size:30px; 
         color: grey; 
   ...
   style="margin-left: 580px;
      ...
      <form action="" method="post">
         ...
         <input type="text" name="id" class="inp form-control text-center w-75 rounded-pill" id="stuid" readonly="" value="<?php echo $_SESSION['staffid'] ?>">
...
{
   $stuid=$_POST['id'];
   $q=" SELECT staffid FROM staff WHERE staffid='$stuid' ";
   if($v=mysqli_query($connect,$q))
   {
      /*echo '<script>window.location.href("s-pupdate.php");</script>';
      print_r($v);*/
      $p=mysqli_fetch_array($v);
      $_SESSION['staff_pass_up_id']=$sendingid=$p[0];
      echo '<script>window.location.href="staff-pupdate.php"</script>';
      #?id="$p[0]"');
      #echo '$p[0]';
      /*print_r($p[0]);*/
   }
