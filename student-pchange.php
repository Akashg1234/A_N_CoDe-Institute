...
<body>
	...
	<div style=" margin-top: 30px;">
		<a href="exameforum.php" class="p-2" 
		style="
		margin-left: 575px;
		font-size:30px; 
		color: grey; 
	...
	style="margin-left: 580px;
		...
		<form action="" method="post">
			<label for="text-center" class="font-weight-bolder" style="font-size: 17px;">Enter Unique Id:</label>
			<input type="text" name="id" class="inp form-control w-75 rounded-pill" id="stuid" required="">
...
{
	$stuid=$_POST['id'];
	$q=" SELECT sid FROM student WHERE sid='$stuid' ";
	if($v=mysqli_query($connect,$q))
	{
		/*echo '<script>window.location.href("s-pupdate.php");</script>';
		print_r($v);*/
		$p=mysqli_fetch_array($v);
		$_SESSION['student_pass_up_id']=$sendingid=$p[0];
		header('Location: s-pupdate.php');
		#?id="$p[0]"');
		#echo '$p[0]';
		/*print_r($p[0]);*/
	}
